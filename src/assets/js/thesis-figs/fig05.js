(function () {
  const FILENAME = 'data/thesis/fig05/percolator-scores.json';
  ...
  // Comet canvas
  const svgComet = d3.select('#figure05-A')
    .append('svg')
  ...
  // MS-GF+ canvas
  const svgMSGF = d3.select('#figure05-B')
    .append('svg')
    .attr('viewBox', `0 0 ${width + margin.left + margin.right} ${height + margin.top + margin.bottom}`)
    .attr('preserveAspectRatio', 'xMinYMin meet')
    // .attr('width', width + margin.left + margin.right)
  ...
  d3.json(FILENAME).then((data) => {
    ...
    function updateDistribution(svg, engine, mixture, plot) {
      ...
      const yScale = d3.scaleLinear()
        .domain(d3.extent(dataFiltered, (d) => d.score)).nice()
        .range([height, 0]);

      // histograms of correct and incorrect identifications
      const correctDist = createHistogram(dataFiltered.map((d) => {
        if (d.correct) return d.score;
      }), plot.xScale, yScale);
      const incorrectDist = createHistogram(data.map((d) => {
        if (!d.correct) return d.score;
      }), plot.xScale, yScale);

      // update histogram of correct identification
      svg.selectAll('rect.correct')
        .data(correctDist.bins)
        .transition()
        .duration(600)
        .attr('width', (d) => correctDist.scale(d.length) - plot.halfBandwidth + plot.offset);

      // update histogram of incorrect identification
      svgMSGF.selectAll('rect.incorrect')
        .data(incorrectDist.bins)
        .transition()
        .duration(600)
        .attr('x', (d) => incorrectDist.scale(-d.length) + plot.offset)
        .attr('width', (d) => plot.halfBandwidth - (incorrectDist.scale(-d.length) + plot.offset));
    }
  });
}());
