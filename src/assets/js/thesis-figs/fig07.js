(function () {
  const DATA_PATH = 'data/thesis/fig07/';
  const DIFF_FILENAME = `${DATA_PATH}differences.csv`;
  const FEATURES = [
    'replicate-spectra', 'precursor-scores', 'sibling-ions',
    'sibling-modifications', 'sibling-scores',
  ];
  ...
  const svgA = d3.select('#figure07-A')
    .append('svg')
  ...
  const svgB = d3.select('#figure07-B')
    .append('svg')
  ...
  d3.csv(DIFF_FILENAME, (d) => ({
    ...
    const plot = engineSwarmPlot(svgA, data, 'difference', {
      width: widthA,
      height: heightA,
      margin: marginA,
    }, 'circle-diff');
    ...
    FEATURES.forEach((feature) => {
      ...
      svgA.insert('rect', labelFeatureKey)
  ...
  function pairedBoxPlot(svg, engine, feature) {
    ...
    d3.json(`${DATA_PATH + key}.json`).then((data) => {
      ...
      svg.selectAll('.box-line-vertical-top')
        .data(stats[key])
        .enter()
        .append('line')
        ...
        .attr('stroke', 'black')
        .style('width', 40);
      svg.selectAll('.box-line-vertical-bottom')
        .data(stats[key])
        .enter()
        .append('line')
  ...
  function computeStats(data) {
    ...
    const coreScores = scores.filter((score) => score >= q1 - 1.5 * iqr
      & score <= q3 + 1.5 * iqr);
