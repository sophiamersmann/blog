...
function plotScoreDistribution(svg, data, config) {
  ...
  const correctDist = createHistogram(data.map((d) => {
  ...
  svg.selectAll('rect.incorrect')
    .transition()
    .duration(300)
    .attr('x', (d) => incorrectDist.scale(-d.length) + offset)
    .attr('width', (d) => halfBandwidth - (incorrectDist.scale(-d.length) + offset))
    .delay((_, i) => i * 100);
