(function () {
  const DATA_PATH = 'data/thesis/fig06/';
  ...
  Promise.all(files.map((f) => d3.json(f))).then((dataChunks) => {
    ...
    REPETITIONS.forEach((rep) => {
      ...
      $(`#${titleId}`).append(
        selectMixtureTemplate.clone().data('rep', rep),
      );
      ...
      scalesCorrect[rep] = setupMultipleCorrect(svgCorrect, dataCorrect, config);
    ...
    figure.selectAll('.select-mixture').on('change', function () {
      ...
      updateMultipleCorrect(
        ...
        dataCorrect[key], scalesCorrect[rep],
      );
      updateMultipleROC(
        `#svg-roc-${rep} > g`,
        dataROC[key], scalesROC[rep],
      );
    });
  });
