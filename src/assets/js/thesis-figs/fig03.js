(function () {
  const DATA_PATH = 'data/thesis/fig03/';
  ...
  const svgA = d3.select('#figure03-A')
    .append('svg')
  ...
  const svgB = d3.select('#figure03-B')
    .append('svg')
  ...
  d3.csv(KS_FILENAME).then((data) => {
    const plot = engineSwarmPlot(svgA, data, 'ks_distance', {
      width: widthA,
      height: heightA,
      margin: marginA,
    }, 'circle-dist');
    ...
    const firstDistObj = data.find((d) => d.dataset === FIRST_DATASET
      && d.engine === FIRST_ENGINE);
    svgA.append('line')
      .attr('id', 'indicator')
    ...
    d3.selectAll('.circle-dist')
      .style('stroke', 'steelblue')
      .style('fill', (d) => ((d.data.dataset === FIRST_DATASET
          && d.data.engine === FIRST_ENGINE) ? 'steelblue' : '#b6cee2'));

    // make data points interactive on hover
    svgA.selectAll('.circle-dist')
      .on('mouseover', function (d) {
        ...
        svgA.select('#indicator').transition()
          .duration(800)
  ...
  function updateDistribution(filename) {
    d3.json(filename, (d) => ({
      ...
      const name = distFilename.substring(
        distFilename.lastIndexOf('/') + 1,
      ).replace('.json', '');
      ...
      const binsCorrect = histogram(data.map((d) => {
        if (d.correct) return d.score;
      }));
      const binsIncorrect = histogram(data.map((d) => {
        if (!d.correct) return d.score;
      }));
      ...
      const violinScaleCorrect = d3.scaleLinear()
        .range([0, xScale.bandwidth()])
        .domain([-maxBinLengthCorrect, maxBinLengthCorrect]);
      const violinScaleIncorrect = d3.scaleLinear()
        .range([0, xScale.bandwidth()])
      ...
      svgB.selectAll('rect.correct')
        .data(binsCorrect)
        .transition()
        .duration(300)
        .attr('width', (d) => violinScaleCorrect(d.length) - haldBandwidth + offset);

      svgB.selectAll('rect.incorrect')
        .data(binsIncorrect)
        .transition()
        .duration(300)
        .attr('x', (d) => violinScaleIncorrect(-d.length) + offset)
        .attr('width', (d) => haldBandwidth - (violinScaleIncorrect(-d.length) + offset));
    });
  }
