...
const selectMixtureTemplate = $('<select>')
  .attr('class', 'select-mixture');
...
function setupMultipleCorrect(svg, data, config, accessor = (d) => d.n_correct) {
  // PEP threshold scale (x-axis)
  const threshScale = d3.scaleLinear()
    .domain(nestedExtent(data, (d) => d.thresh)).nice()
    .range([0, config.width]);

  // Number of correct identifications (y-axis)
  const yScale = d3.scaleLinear()
    .domain(nestedExtent(data, accessor)).nice()
...
function updateMultipleCorrect(svgSele, data, scales) {
  ...
  METHODS.forEach((method) => {
    ...
    svg.select(`.band-${method}`)
      .datum(dataFiltered)
      .transition()
      .duration(1000)
      .attr('d', d3.area()
        .x((d) => scales.threshScale(d.thresh))
        .y0((d) => scales.yScale(math.mean(d.n_correct) - math.std(d.n_correct)))
        .y1((d) => scales.yScale(math.mean(d.n_correct) + math.std(d.n_correct)))
        .curve(d3.curveMonotoneX));

    // update mean line plot
    svg.select(`.mean-${method}`)
      .datum(dataFiltered)
      .transition()
      .duration(1000)
      .attr('d', d3.line()
