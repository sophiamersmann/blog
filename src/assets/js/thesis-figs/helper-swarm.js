function engineSwarmPlot(svg, data, xKey, config, circleSel, radius = 7) {
  ...
  const engineScale = d3.scaleBand()
  ...
  const cell = svg.append('g')
    ...
    .data(d3.voronoi()
      .extent([
        ...
        [config.width + config.margin.right, config.height + config.margin.top],
      ])
      .x((d) => d.x)
      .y((d) => d.y)
      .polygons(data))
